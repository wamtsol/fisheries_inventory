            ...
            <div class="col-sm-10">
                <select name="customer_id" id="customer_id" class="margin-btm-5 selectbox">
					...
					if( numrows( $rs ) > 0 ) {
						while( $r = dofetch( $rs ) ) {
							?>
							<option value="<?php echo $r[ "id" ]?>" data-customer_name="<?php echo htmlspecialchars(unslash($r[ "customer_name" ]))?>" data-phone="<?php echo htmlspecialchars(unslash($r[ "phone" ]))?>" data-address="<?php echo htmlspecialchars(unslash($r[ "address" ]))?>"<?php echo $customer_id==$r["id"]?" selected":""?>><?php echo unslash($r[ "customer_name" ])?></option>
							<?php
						}
					}
					?>
