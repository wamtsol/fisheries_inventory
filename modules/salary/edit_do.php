...
if(isset($_POST["salary_edit"])){
	...
	if($err==""){
		$sql="Update salary set `employee_id`='".slash($employee_id)."', `month`='".slash($month)."', `year`='".slash($year)."', `datetime_added`='".slash(datetime_dbconvert($datetime_added))."', `amount`='".slash($amount)."' where id='".$id."'";
		doquery($sql,$dblink);
